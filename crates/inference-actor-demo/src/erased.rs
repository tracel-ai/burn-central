use crate::runtime::{Job, RequestId, SessionHandle};
...
pub trait ErasedSession: Send + Sync {
    fn spec(&self) -> &'static InferenceSpec;
...
pub struct JsonSession<I, O, E> {
    ...
    spec: &'static InferenceSpec,
}

impl<I, O, E> JsonSession<I, O, E> {
    pub fn new(inner: SessionHandle<I, O, E>, spec: &'static InferenceSpec) -> Self {
        Self { inner, spec }
    }
}
